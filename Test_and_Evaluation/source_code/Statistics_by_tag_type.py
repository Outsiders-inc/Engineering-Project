...
percent_str = "PERCENT"
misc_str = "MISC"
NUM_OF_DIFFERENT_TAGS = 6

_correct_arr = [0] * NUM_OF_DIFFERENT_TAGS
_incorrect_arr = [0] * NUM_OF_DIFFERENT_TAGS
_missing_arr = [0] * NUM_OF_DIFFERENT_TAGS
_spurious_arr = [0] * (NUM_OF_DIFFERENT_TAGS + 1)

#tagType = Enum(PERSON = 0, ORGANIZATION = 2, LOCATION = 2, DATE = 3, PERCENT = 4, MISC = 5)
tagTypeMap = {'PERS' : 0, 'ORGA' : 1, 'LOC' : 2, 'DATE' : 3, 'PERCENT' : 4, 'MISC' : 5}
currentType = tagTypeMap['PERS']
prevType = tagTypeMap['PERS']
...
def analyze_sentence(sent):
    ...
    global _correct_arr
    global _incorrect_arr
    global _missing_arr
    global _spurious_arr
    ...
    for i, word in enumerate(sent):
        ...
        # Finding the exact current tag:
        prevType = currentType
        # if i > 0:
        for key in tagTypeMap:
            # print("Key: " + key)
            # word_tmp = sent[i-1].split("\t")
            # if key in word_tmp[GOLD_STANDARD_TAG]:
            print("key: " + key + ", GOLD_STANDARD_TAG: " + word[GOLD_STANDARD_TAG])
            if key in word[GOLD_STANDARD_TAG]:
                currentType = tagTypeMap[key]
                break
        ...
        if word[GOLD_STANDARD_TAG] == NOT_A_NAME:
            if continuing_length >= 1:
                if continue_correct == correct:
                    _correct += 1
                    _correct_arr[prevType] += 1
                elif continue_correct == incorrect:
                    _incorrect += 1
                    _incorrect_arr[prevType] += 1
                elif continue_correct == missing:
                    _missing += 1
                    _missing_arr[prevType] += 1
            ...
            if word[OUR_CRF_TAG] == NOT_A_NAME: # or word[OUR_CRF_TAG] == END_OF_SENTENCE:
                ...
                _spurious += 1
                # Calculation which spurious tag we've guessed
                ourTagType = NUM_OF_DIFFERENT_TAGS
                for key in tagTypeMap:
                    if key in word[OUR_CRF_TAG]:
                        ourTagType = tagTypeMap[key]
                        break
                _spurious_arr[ourTagType] += 1
            total_phrases_in_sentence += 1
...
def make_statistics(res_file, _total_phrases, _total_names):
    ...
    print("---------------------------")
    print("\t\t\t\t\tPERS\tORG\t\tLOC\t\tDATE\tPERCENT\tMISC\tUnrecognizable_Spurious")
    print("_correct_arr: \t\t" + "\t\t".join(map(str, _correct_arr)))
    print("_incorrect_arr: \t" + "\t\t".join(map(str, _incorrect_arr)))
    print("_missing_arr: \t\t" + "\t\t".join(map(str, _missing_arr)))
    print("_spurious_arr: \t\t" + "\t\t".join(map(str, _spurious_arr)))
