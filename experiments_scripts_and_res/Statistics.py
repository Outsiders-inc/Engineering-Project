...
def init_globals():
    global _F_by_sent
    _F_by_sent = []
    global recall_by_sent
    recall_by_sent = []
    global _precision__by_sent
    _precision__by_sent = []
    global _total_recall
    _total_recall = 0
    global _total_precision
    _total_precision = 0
    global _total_F_measure
    _total_F_measure = 0
    global _correct_list
    _correct_list = []
    global _incorrect_list
    _incorrect_list = []
    global _missing_list
    _missing_list = []
    global _spurious_list
    _spurious_list = []
    global _O_positive_list
    _O_positive_list = []
...
def run_analysis(res_file):
    ...
    #  Check if end_of_file is True, to know if we've scanned all of the file!!!
    if not end_of_file:
        exit("ERROR!! DID NOT REACH END OF FILE!")
    make_statistics(res_file, _total_phrases, _total_names)

    # Write to file
    res_file_name = res_file + "_analysis" + ".txt"
    res = open(res_file_name, 'w', encoding="utf-8")
    res.write(">>> TOTAL : <<< \n")
    res.write("    ------      \n")
    res.write("  F_measure     \n")
    res.write(str(_total_F_measure) + "\n")
    res.write("    Recall      \n")
    res.write(str(_total_recall) + "\n")
    res.write("    Precision      \n")
    res.write(str(_total_precision) + "\n")
